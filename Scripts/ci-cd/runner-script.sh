...
if [ -n "$health_check_link" ];
    then
            ...
            if curl -m 2 -s "$health_check_link" | grep OK ;
                ...
                if [[ $health_check_counter == @(600|1200) ]]
                then
                    ...
                    ssh "$target_server" -o ConnectTimeout=15 docker logs --tail 100 "$service_name"
                else
        ...
        echo -e "$GRN \U2705 ################# Health Check Undefined #################\n$NC"
fi